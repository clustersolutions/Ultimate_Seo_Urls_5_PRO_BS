<?php
  ...
  * @Id $Id:: Usu_Template.php 199 2011-01-26 15:27:34Z Rob             $:  Full Details   
  */
  ...
  */
  class Xxx_Page_Module extends aPage_Modules {
    ...
    */
    protected $dependencies = array( 'xxx_id' => array( 'marker'     => '-xxx-', // like -p- or -c- etc
                                                        'query'      => "SELECT xxx FROM :TABLE_XXX WHERE xxx_id=':xxx_id' AND language_id=':languages_id' LIMIT 1",
                                                        'to_replace' => array( ':TABLE_XXX', ':xxx_id', ':languages_id' ) ) ); // TABLE_XXX and xxx_id must match the line above
    /**
    ...
    */
    protected $cache_name_builder = array( 'xxx_id' => 1 ); // xxx = _GET key ( e.g. cPath ), you may want to add "page" if there are paging functions
    /**
    * Class constructor
    * @access private
    */
    ...
    */
    public function buildLink( $page, $parameters, $add_session_id, $connection ) {
      $extract = array( 'xxx_id' ); // xxx = _GET key ( e.g. cPath )
      ...
      */
      ...
      switch ( true ) {
        case $this->key == 'xxx_id': // xxx = _GET key ( e.g. cPath )
          // This array contains replacements for the to_replace array ( see the $dependencies array )
          $this->setQuery( array( TABLE_XXX, $this->stripPathToLastNumber( $this->keys_index[$this->key] ), Usu_Main::i()->getVar( 'languages_id' ) ) );
          break;
        default:
